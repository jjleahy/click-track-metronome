...
describe('MeasureCard', () => {
  it('renders 4 subdivision inputs for 4/4', () => {
    render(<MeasureCard {...defaultProps} measure={make44()} />);
    const inputs = screen.getAllByRole('spinbutton', { name: /beat \d+ subdivisions/i });
    expect(inputs).toHaveLength(4);
  });

  it('does not have invalid class when valid', () => {
    const { container } = render(<MeasureCard {...defaultProps} measure={make44()} />);
    expect(container.firstChild).not.toHaveClass('measure-card--invalid');
  });
  ...
  it('has invalid class when sum does not equal numerator', () => {
    ...
    const { container } = render(<MeasureCard {...defaultProps} measure={measure} />);
    expect(container.firstChild).toHaveClass('measure-card--invalid');
  });

  it('has active class when isActive', () => {
    const { container } = render(
      <MeasureCard {...defaultProps} measure={make44()} isActive={true} />
    );
    expect(container.firstChild).toHaveClass('measure-card--active');
  });

  it('numerator change resets beats via defaultBeats', () => {
    const onChange = vi.fn();
    render(<MeasureCard {...defaultProps} measure={make44()} onChange={onChange} />);
    const numInput = screen.getByRole('spinbutton', { name: /time signature numerator/i });
    fireEvent.change(numInput, { target: { value: '6' } });
    // onChange should have been called with 6/4 and defaultBeats(6,4)
    const lastCall = onChange.mock.calls[onChange.mock.calls.length - 1][0] as Measure;
  ...
  it('subdivision overflow trimming calls onChange with trimmed beats', async () => {
    ...
    render(<MeasureCard {...defaultProps} measure={measure} onChange={onChange} />);
  ...
  it('delete button calls onDelete', async () => {
    ...
    render(<MeasureCard {...defaultProps} measure={make44()} onDelete={onDelete} />);
  ...
  it('insert-after button calls onInsertAfter', async () => {
    ...
    render(<MeasureCard {...defaultProps} measure={make44()} onInsertAfter={onInsertAfter} />);
  ...
  it('canDelete=false disables delete button', () => {
    render(<MeasureCard {...defaultProps} measure={make44()} canDelete={false} />);
  ...
  it('clearing tempo input on blur sets tempo to null', async () => {
    const onChange = vi.fn();
    const measure: Measure = { ...make44(), tempo: 120 };
    render(<MeasureCard {...defaultProps} measure={measure} onChange={onChange} />);
    const tempoInput = screen.getByRole('spinbutton', { name: /tempo/i });
    await userEvent.clear(tempoInput);
    await userEvent.tab();
